...
meta.file <- "MetaData/SampInfo_wMetaData_withUTC.csv"
...
dat.hilicpos.file.nostds <- "RawOutput/20200430_Check/HILICPos_IntegrationsBigPeaksWTargeted2.csv"
dat.hilicneg.file.nostds <-  "RawOutput/20200430_Check/HILICNeg_IntegrationsBigPeaksWTargeted.csv"
dat.cyano.file.nostds <- "RawOutput/20200430_Check/CyanoAq_IntegrationsBigPeaksWTargeted.csv"
...
dat <- read_csv(dat.file1) %>%
  left_join(read_csv(mf.file) %>% select(MassFeature_Column, z), by = "MassFeature_Column") %>%
...
dat.with.Quan <- dat.withRFs %>%
  mutate(nmolinEnviroave = Adjusted_Area/RF/RFratio*10^-6*400/Volume*1000*Dilution_Factor) %>%
...
IS_dat <- rbind(dat1, dat2) %>%
  ...
  select(SampID, IS, IS_Area, Identification, Concentration_nM)
...
datsmp.quan.newIS <- dat.with.Quan %>%
  ...
  mutate(nmolinEnviroave_IS = umolinvial_IS*10^-6*400/Volume*Dilution_Factor)
...
datsmp.quan.IScorrected <- dat.with.Quan %>%
  ...
  mutate(RFFlag = ifelse(is.na(nmolinEnviroave_IS), RFFlag, "Matched IS"))%>%
  select(MassFeature:nmolinEnviroave, RFFlag)
