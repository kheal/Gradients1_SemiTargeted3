...
samp.info.short <- samp.info %>%
  select(CultureID, Species, strain, Org_Type, CultureID_short, BioVol_perFilter_uL, 
         #volume_reconst, HILIC_injectVol, RP_injectVolume, 
         nmolC_filtered_final, Method) %>%
  rename(`Sample ID` = CultureID,
         `Broad taxon` = Org_Type, Strain = strain, 
         `Short ID` = CultureID_short, 
         `Biovolume filtered (uL)` = BioVol_perFilter_uL, 
