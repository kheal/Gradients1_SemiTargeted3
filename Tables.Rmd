...
```{r, error = FALSE, warning=FALSE, message=FALSE}
library(openxlsx) #requires Java between 8 and 11
...
for (i in 1:length(supp.tables)) {
  ...
  addWorksheet(wb, sheetName=files.nms.2$sheet_ID[i])

  ## output the csv contents
  writeData(wb, sheet = files.nms.2$sheet_ID[i], x=temp_DT, keepNA = T, na.string = "NA")
  #, sheet=i, header=TRUE, rownames=NULL)
}

saveWorkbook(wb, f.out)
remove(list = ls())
