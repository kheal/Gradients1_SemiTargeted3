...
dat2 <- dat %>%
  select(BestMatch, SampID, molFractionN)
...
AllSmps_long_Summary <- AllSmps_long %>%
  ...
  summarise(Latitude = round(mean(latitude), digits = 1),
            molFractionN = mean(as.numeric(molFractionN), na.rm = T))%>%
  ungroup() %>%
...
Others <- AllSmps_long_Summary %>%
  ...
  summarise(measured = sum(molFractionN, na.rm = T)) %>% ungroup() %>%
  mutate(molFractionN = 1-measured,
...
AllSmps_long_Summary_Tran <- AllSmps_long_Summary_withOthers %>%
  ...
  filter(!is.nan(molFractionN))

b <- ggplot(AllSmps_long_Summary_Tran, aes(x = Latitude, y = molFractionN, fill = MF_Frac))+
...
c <- ggplot()+
  geom_bar(data = AllSmps_long_Summary_nDP  %>% filter(Depth == "40"), 
           aes(x = as.numeric(Depth), y = molFractionN, fill = MF_Frac), 
  ...
  labs(y= "mole fraction N", x = "Depth")+
  guides(fill=FALSE) +
...
d <- ggplot(AllSmps_long_Summary_sDP, aes(x = as.numeric(Depth), y = molFractionN, fill = MF_Frac))+
  scale_x_reverse() +
  scale_y_continuous(expand = c(0, 0), limits = c(0,1))+
...
save_plot("Figures/Manuscript_figures/molPerCent_N.pdf", b_c_d2, base_height = 6, base_width = 6.5, units = "in")  
